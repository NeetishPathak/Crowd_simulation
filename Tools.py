...
def angleVec(vec1, vec2):
	result = -(atan2(vec1.y, vec1.x) - atan2(vec2.y, vec2.x)) * 180.0/pi;
	if(result < 0):
		result += 360.0;
	return result
...
class line(object):
	...
	def intersectionWithLine(self,line):
		...
		start2 = line.start
		end2 = line.end
		denom =  (end2.y -start2.y)*(end1.x -start1.x)- (end2.x -start2.x )*(end1.y -start1.y)
		if denom!= 0 :
			a = ((end2.x -start2.x)*(start1.y -start2.y)- (end2.y -start2.y)*(start1.x -start2.x))/denom
			b = ((end1.x -start1.x)*(start1.y -start2.y)- (end1.y -start1.y)*(start1.x -start2.x))/denom
	...
	def addedge(self, n1, n2,label=''):
		...
		try:
			if not (v in self.nodes[u] or u in self.nodes[v]):
			    ...
			    wt = mag(vector(u[0],u[1])-vector(v[0],v[1]))
